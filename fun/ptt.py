...
def ptt_crawl_one(Board_Name):
    ...
    for summary in latest_page: # 只要抓最新的頁面
        ...
        if article !='':        
            ...
            links.append('https://www.ptt.cc' + article.url)
...
class ArticleListPage(Page):
    ...
    @classmethod
    def from_board(cls, board, index='', name= ''):
        ...
        # url = '/'.join(['/bbs', board, 'search?page='+str(index)+ '&q=' + name + '.html'])
        return cls(url)
...
def crawl_ptt_page_auto(Board_Name, page_num):
    ...
    import re
    lst_page = ArticleListPage.from_board(board =Board_Name)
    start = lst_page.related_urls['previous']
